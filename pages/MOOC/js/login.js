...
function dialogRegister(jumpType) {
    ...
    var emailVal = $("#u-email-reg").val();
    var passwdVal = $("#u-password-reg").val();
    var passwdRep = $("#u-password-rep").val();
    ...
    */
    if (emailRegister == "ON") {
        ...
        || !emailVal.match(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/)
        ) {
            // $("#u-email-reg").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>���������䣡</span>');
            $("#u-email-reg").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入正确邮箱地址</span>');
    ...
    if (passwdVal == null || passwdVal == '') {
        $("#u-password-reg").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入密码</span>');
        $("#u-password-reg").parent().addClass("err");
        return
    }
    if (passwdVal != passwdRep){
        $("#u-password-rep").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>两次密码不一致</span>');
        $("#u-password-rep").parent().addClass("err");
        return
    }
    ...
    $.ajax({
        // url: baselocation + "/user/ajax/createuser",
         url: homeAddress()+"one/mainmooc/register.template",
        data: {
        ...
        success: function (result) {
            if (result.success == 1) {
                if (jumpType == 'ucIndex') {
                    ...
                    login(emailVal, passwdVal, passResult);  //登录login
                }
            } else {
                /*�������֤�����*/
        ...
        error: function (error) {
            // $(".e-l-jy").html('<font class="fsize12 c-orange">ϵͳ��æ�����Ժ��ٲ��� 错误</font>');
            $(".e-l-jy").html('<font class="fsize12 c-orange"> 服务器繁忙,请稍后再试</font>');
