...
//edit by tangmengdi			.addClass('ke-container ke-container-' + self.themeType).css('width', width);
		if (fullscreenMode) {
			container.css({
				position : 'absolute',
				left : 0,
...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function bindEvent(el, result, data, createFunc) {
			var fileUrl = K.formatUrl(result.current_url + data.fileId, 'absolute'),
		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				div.append(photoDiv);
				var fileUrl = result.current_url + data.thumbnail,
